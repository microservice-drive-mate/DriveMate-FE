...
export default function Home() {
	...
	const { notifications, refresh: refreshNotifications } =
	...
	useEffect(() => {
		let cancelled = false;
		const load = async () => {
			const [analyticsResult, historyResult, streakResult] =
				await Promise.all([
			...
			if (historyResult.success)
				setRecentAttempts(historyResult.items.slice(0, 3));
	...
	const licenseTier =
		user?.role === UserRole.STUDENT
			? (user?.studentDetail?.licenseTier ?? null)
			: null;
	const vehicleLabel = licenseTier
		? (LICENSE_VEHICLE[licenseTier] ?? "")
		: "";
...
const styles = StyleSheet.create({
	...
	progressBarFill: {
		height: vs(6),
		backgroundColor: AUTH_UI.colors.accentText,
	},
